...
**Formula** (replace the emoji 👀 with this number). `salt_032(👀 + sum(avg(nn(A.hash) + nn(B.hash))) OVER ()) AS token`
...
**Formula** (replace the emoji 👀 with this sum). `salt_046(👀 + sum(avg(nn(hash))) OVER ()) AS token`
